...
function output = list_parameter()
    ...
    for index = 1:len_dict
        ...
        set_param(line_marked{index}, 'Name', [par_name{index} '*']);
    end
    par_name = par_name';
    ...
    for i = 1:length(all_line)
        ...
        try
            if strcmp('*', current_line_name(end))
                % Revert the line's name
                set_param(all_line(i), 'Name', current_line_name(1:(end-1)));
            else
    ...
    table_name_dict = {'No.', 'Old Name', 'New Name'};
    xlswrite(filename, table_name_dict, 'dict', 'A1');
    xlswrite(filename, table_name_dict, 'only_line', 'A1')
    table_name_simu_par = {'No.', 'Name', 'Data Type', 'Value'};
    xlswrite(filename, table_name_simu_par, 'simu_parameter', 'A1');
    table_name_table = {'No.', 'Name', 'Data Type',...
                        'Row', 'Column','Value'};
    ...
    if 1 < j
        ...
        report_message = {report_message; 'This model has no line name which'...
                          'only defined on line.\n'};
    end
    ...
    if 1 < index_par
        ...
        xlswrite(filename, simu_par_datatype', 'simu_parameter', 'C2');
        xlswrite(filename, simu_par_value', 'simu_parameter', 'D2');
    end

    % Write table data to excel
    if 1 < index_table
        temp_dim = 0;
        for i = 1:length(simu_table)
            % Calculate write position
            temp_pos = num2str(2 + temp_dim);
            table_num_position = ['A' temp_pos];
            table_name_position = ['B' temp_pos];
            table_datatype_position = ['C' temp_pos];
            table_dim_position = ['D' temp_pos];
            % table_dim_column_position = ['E' temp_pos];
            table_value_position = ['F' temp_pos];
            % Start writing
            ...
            temp_dim = temp_dim + simu_table{i}{3}(1);
        end
    end
